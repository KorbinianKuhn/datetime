...
describe('unix()', () => {
  ...
  dates.map(date => {
    it(`should return correct unix timestamp with date ${
      typeof date === 'object'
        ? date.toISOString() + ' type date'
        : date + ' type string'
    }`, () => {
      const expected = moment(date).unix();
      const actual = datetime(date).unix();
      expect(actual).toEqual(expected);
    });

    it(`utc: should return correct unix timestamp with date ${
      typeof date === 'object'
        ? date.toISOString() + ' type date'
        : date + ' type string'
    }`, () => {
      const expected = moment.utc(date).unix();
      const actual = datetime.utc(date).unix();
      expect(actual).toEqual(expected);
    });
  });
});
