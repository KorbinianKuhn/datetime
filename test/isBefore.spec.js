...
describe("isBefore functions", () => {
  ...
  const functionNames = ['isBefore', 'isBeforeMillisecond', 'isBeforeSecond', 'isBeforeMinute', 'isBeforeHour', 'isBeforeDay', 'isBeforeMonth', 'isBeforeYear']
  ...
  for (let i = 0; i < functionNames.length; i++) {
    ...
    for (const dateB of dates) {
      const expected = moment(dateA)
        .isBefore(moment(dateB), momentInterval);
