...
describe("isSameOrBefore functions", () => {
  ...
  const functionNames = ['isSameOrBefore', 'isSameOrBeforeMillisecond', 'isSameOrBeforeSecond', 'isSameOrBeforeMinute', 'isSameOrBeforeHour', 'isSameOrBeforeDay', 'isSameOrBeforeMonth', 'isSameOrBeforeYear']
  ...
  for (let i = 0; i < functionNames.length; i++) {
    ...
    for (const dateB of dates) {
      const expected = moment(dateA)
        .isSameOrBefore(moment(dateB), momentInterval);
