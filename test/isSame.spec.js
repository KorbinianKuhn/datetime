    ...
    dates.map(date1 => {
      dates.map(date2 => {
        it(`with unit ${unit} and date1 ${date1.toISOString()} and date2 ${date2.toISOString()} should verify`, () => {
          const expected = moment(date1).isSame(date2, unit);
          const actual = datetime(date1).isSame(date2, unit);
          expect(actual).toEqual(expected);
        });

        it(`utc: with unit ${unit} and date1 ${date1.toISOString()} and date2 ${date2.toISOString()} should verify`, () => {
          const expected = moment.utc(date1).isSame(date2, unit);
          const actual = datetime.utc(date1).isSame(date2, unit);
          expect(actual).toEqual(expected);
        });
      });
    });
